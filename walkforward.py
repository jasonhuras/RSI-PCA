...
def pca_linear_model(
    x: pd.DataFrame, y: pd.Series, n_components: int, thresh: float = 0.01
):
    # Center data at 0
    means = x.mean()
    x -= means
...
def pca_rsi_model(
    ...
    for i in range(next_train, len(ohlc)):
        if i == next_train:
            ...
            model_coefs, evecs, rsi_means, l_thresh, s_thresh, _ = pca_linear_model(
                train_data, y, n_components
            )
...
if __name__ == "__main__":
    ...
    print(
        "Mean Target Above Long Threshold",
        output[output["pred"] > output["long_thresh"]]["t"].mean(),
    )
    print(
        "Mean Target Below Short Threshold",
        output[output["pred"] < output["short_thresh"]]["t"].mean(),
    )
    ...
    # Profit fac
    pf = (
        data[data["strat_ret"] > 0]["strat_ret"].sum()
        / data[data["strat_ret"] < 0]["strat_ret"].abs().sum()
    )
